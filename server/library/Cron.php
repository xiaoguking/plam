...
class Cron
{
    ...
    protected function checkSecond(\DateTime $now)
    {
        if ($this->parts['sec'] != '*') {
            foreach ($this->parseRule($this->parts['sec'], 0, 59) as $value) {
                if ($now->format('s') == $value) {
                    return true;
                }
            }
    ...
    protected function parsePattern($pattern)
    {
        ...
        $regex = [];
        foreach (array_slice($parts, 0, 6) as $name => $number) {
            $range = '(' . $number . ')(-(' . $number . '))?';
